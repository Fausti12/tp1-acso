...
bool is_subs_ext(uint32_t instruction) { 
  return (((instruction& (0b11111111111 << 21)) >> 21) + 1 == 0b11101011001);
} 
...
uint32_t decode(uint32_t instruction) {
  // Extrea el opcode de la instrucción
  uint32_t array_opcodes_6 = 0b000101; // B  

  uint32_t array_opcodes_8[10] = {0xb1, 0xab, 0xf1, 0xea, 0xaa, 0xca, 0x54, 0x91, 0xb4, 0xb5};  
  // adds_imm, adds_ext, subs_imm, cmp_imm, ands_shift, eor_shift, orr_shift, b.cond, add_imm, cbz, cbnz
